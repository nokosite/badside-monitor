...
export async function GET() {
  const servers = listServers().map(s => {
    const snaps = readSnapshots(s.id);
    return {
      ...s,
      snapshot_count: snaps.length,
      last_fetch: snaps.length ? snaps[snaps.length - 1].fetched_at : null,
    };
  });
...
export async function POST(req: Request) {
  ...
  addServer({ id, name: name.trim(), cfx_code: cfx_code.trim(), created_at: new Date().toISOString() });
